...
public class Ejercicio1 {
	 ...
	 * 
	 * -The first if checks if the number that the user wrote was in the limit, to
	 * test it out, we introduce a bigger number, and a lower number than the ones
	 * that are allowed.
	 * 
	 * -The second conditional checks if the number has only one digit, to test it
	 * we introduce a number of one digit, and then a number of two digits. With one
	 * digit the program has to always answer that the number is palindromic
	 * 
	 * -The third conditional checks if the number has two digits and if it is
	 * palindromic, we have to introduce numbers with more digits, with less and
	 * introducing palindromic and not palindromic numbers
	 * 
	 * -We will do the same tests with each one of the following conditionals than
	 * with the ones with three.
	 * 
	...
	public static void main(String[] args) {
		...
		if (num < 0 || num > 9999) { // If the number is not in the limits, this condition write it down
		...
		} else if (num < 1000 && num / 100 == num % 10) { // If the number has 3 digits, we watch if the first and third
															// digits are the same
			...
			System.out.println("El número introducido es capicúa"); // If the first and the fourth & the second and the
																	// third are the same digits, we write it down
